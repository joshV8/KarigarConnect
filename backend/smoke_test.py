...
Tests the full end-to-end artisan flow against any target environment:
  ...
  API_URL=https://artisan-api.onrender.com AUTH_TOKEN=my_firebase_token python smoke_test.py
"""
...
def run_smoke_test():
    ...
    if enquiry_id:
        ...
        # PUT enquiry: update status and add artisan response
        r = requests.put(
            f"{API_URL}/api/v1/enquiries/{enquiry_id}",
            headers=HEADERS,
            json={"status": "contacted", "artisan_response": "We have sample units ready for dispatch."},
            timeout=10,
        )
